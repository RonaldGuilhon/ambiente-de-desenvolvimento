...
class PostgresStatusWorker(QObject):
    ...
    def run(self) -> None:
        ...
        result = self._pm.run_command(
            ["sc", "query", "postgresql"], timeout=10
        )
        if result.success and "RUNNING" in result.stdout.upper():
            ...
            info.status = PostgresStatus.UNKNOWN

        ver_result = self._pm.run_command(
            ["psql", "--version"], timeout=5
        )
...
class PostgresService(QObject):
    ...
    def check_status_async(self) -> None:
        ...
        worker = PostgresStatusWorker(self._pm)
